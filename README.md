...
## 27. Two-minute viva explanation

AI RiskGuard is an educational digital payment risk management prototype for a
Razorpay-style payment risk use case. It uses only synthetic and anonymized data,
and it is not an official Razorpay product.

The project generates 12,000 reproducible transactions with behavioral features
such as amount, payment method, new-device status, failed attempts, transaction
velocity, merchant risk, location change, account age, time, and international
status. Fraud labels are probabilistic and noisy, so the target is imbalanced
without simply copying the rule engine.

During training, I engineer features such as the ratio between the current amount
and the user's historical average. I split with stratification before fitting
preprocessing, which prevents leakage. Numerical fields use median imputation and
scaling; categorical fields use unknown-safe one-hot encoding. Logistic Regression
and Random Forest are evaluated using precision, recall, F1, ROC-AUC, false-positive
rate, false-negative rate, and a confusion matrix. The automatic selection rule
gives recall and F1 the highest importance.

During inference, the selected model produces a fraud probability. A separate rule
engine adds understandable signals such as a new device, high velocity, failed
attempts, amount deviation, or merchant risk. The final score combines 70% ML risk
and 30% rule risk, then returns LOW, MEDIUM, or HIGH. Results are stored in SQLite,
and HIGH risk creates a manual-review alert. Streamlit provides monitoring,
filtering, alerts, and actual held-out metrics. The system never authorizes or
blocks a real payment.

## 28. Likely interview questions

1. **Why did you build this project?**
   To demonstrate an end-to-end payment-risk workflow covering data, ML, rules,
   explainability, persistence, UI, testing, and responsible scope.

2. **Why is accuracy not enough?**
   Fraud is rare, so predicting everything as genuine can give high accuracy while
   detecting no fraud. Recall, precision, F1, and ROC-AUC are also required.

3. **How did you prevent data leakage?**
   I split before fitting preprocessing, exclude IDs and the target, and reuse the
   train-fitted preprocessor during inference.

4. **Why use stratification?**
   It preserves the fraud/genuine ratio in both training and test partitions.

5. **Why train two models?**
   Logistic Regression is a clear linear baseline; Random Forest can learn
   nonlinear interactions. Held-out metrics support an evidence-based choice.

6. **How is the selected model chosen?**
   A composite weights recall and F1 at 35% each, ROC-AUC at 20%, and precision at 10%.

7. **What is `amount_ratio`?**
   Current amount divided by historical average, with an epsilon to prevent
   division by zero. It captures deviation from normal behavior.

8. **Why combine ML and rules?**
   ML captures multivariable patterns; rules provide understandable indicators.
   The combination balances predictive and explanatory value.

9. **How is the final score calculated?**
   `0.70 × ML risk score + 0.30 × normalized rule score`, clamped to 0–100.

10. **What does the confusion matrix show?**
    TN and TP are correct decisions, FP is genuine traffic incorrectly flagged,
    and FN is fraud missed by the model.

11. **How do you handle unknown categories?**
    One-hot encoding uses `handle_unknown="ignore"`; manual UI input is also
    validated against the allowed categories.

12. **What is stored in SQLite?**
    Anonymous transaction fields, scores, level, recommendation, rule factors,
    timestamps, and HIGH-risk alerts—never full payment credentials.

13. **Can this system calculate live accuracy?**
    No. Manual predictions have no ground-truth outcome. Only held-out test metrics
    are reported as model performance.

14. **What are the main limitations?**
    Synthetic data, uncalibrated baseline probabilities, static rules, no live
    outcome feedback, and no production security or authorization layer.

15. **What would you improve first?**
    Collect review outcomes, calibrate probabilities, tune thresholds using business
    costs, and add time-aware validation before trying more complex models.
